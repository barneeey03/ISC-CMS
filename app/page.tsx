  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === "admin") {
        ...
        if (SUPER_ADMIN_USERS[email as keyof typeof SUPER_ADMIN_USERS] === password) {
          isValidUser = true;
        }
      }
      ...
      if (isValidUser) {
        ...
        localStorage.setItem(
          "auth",
          JSON.stringify({ email, role: userRole, isAuthenticated: true })
        );
        router.push(userRole === "admin" ? "/admin/dashboard" : "/super-admin/dashboard");
      } else {
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="bg-white rounded-xl shadow-2xl p-8">
          ...
          <div className="text-center -mt-2">
            ...
            <div className="mt-2 h-1 w-16 bg-gradient-to-r from-[#60A0C0] to-[#0080C0] mx-auto rounded-full"></div>
          </div>

          {!role ? (
            <div className="mt-8 space-y-3">
              <p className="text-center text-sm font-semibold text-[#002060] mb-4">
              ...
              <button
                ...
                className="w-full px-4 py-3 bg-[#002060] hover:bg-[#001840] text-white font-semibold rounded-lg transition-colors duration-300 text-sm"
              >
                Super Admin Login
              </button>
            </div>
            ...
            <>
              ...
              {error && <p className="text-center text-red-600 text-sm mb-4 mt-4">{error}</p>}

              <form onSubmit={handleLogin} className="space-y-5 mt-6">
                {/* Role Info */}
                <div className="px-3 py-2 bg-[#E8F4F8] rounded-lg border border-[#B0D8E8]">
                  <p className="text-xs font-semibold text-[#0080C0]">
                    Logging in as: {role === "admin" ? "Admin" : "Super Admin"}
                  </p>
                </div>
                ...
                <button
                  ...
                  className="w-full px-4 py-2.5 border border-[#D0E0F0] text-[#0080C0] font-semibold rounded-lg hover:bg-[#F9FBFD] transition-colors duration-300 text-sm"
                >
